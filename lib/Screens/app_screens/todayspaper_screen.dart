            ...
            Container(
              ...
              child: Row(
                children: <Widget>[
                  ...
                  SizedBox(width: 10.0),
                  Text(posterName, style: TextStyle(fontSize: 20.0, fontWeight: FontWeight.bold),)
                ],
              ),
            ),
            Divider(
    ...
    return new Card(
      ...
      child: Column(
        children: <Widget>[
          ...
          Text(_allCategories[index].name, style: TextStyle(fontSize: 20.0),),
          SizedBox(height: 10.0)
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
      backgroundColor: Colors.blue,
        appBar: new AppBar(centerTitle: true, title: new Column(children: <Widget>[SizedBox(height: 3.0,),Icon(Icons.local_library), Text('Today\'s Paper in ' + globals.selectedCity)],), automaticallyImplyLeading: false,),
        body: new Stack(
          children: <Widget>[
            Container(
              height: 100.0,
            ...
            Container(
              ...
              child: StreamBuilder(
                stream: Firestore.instance.collection("Cities").document(globals.selectedCity).collection("Posts").where("Category", isEqualTo: globals.selectedCategory).snapshots(),
                      ...
                      switch (postSnapshot.data.documents[index]["Type"]) {
                        case "Image":
                          return new ImageCard(
                            imageLink: postSnapshot.data.documents[index]["ImageLink"],
                            imageDescription: postSnapshot.data.documents[index]["ImageDescription"],
                            posterName: postSnapshot.data.documents[index]["PosterName"],
                          );
                          break;
                        
                        case "Text":
                          return new TextCard(
                            postText: postSnapshot.data.documents[index]["PostText"],
                            posterName: postSnapshot.data.documents[index]["PosterName"],
                          );
                          break;
/*
